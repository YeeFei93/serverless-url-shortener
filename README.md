...
│   ├── Dockerfile.shorten    # Docker container for shorten function
│   ├── Dockerfile.redirect   # Docker container for redirect function
│   ├── Dockerfile.options    # Docker container for options function
│   └── (Docker images built via docker-build.sh)
...
│   ├── monitor.yml       # Infrastructure monitoring playbook
│   └── inventory         # Ansible inventory configuration
├── frontend/
│   └── index.html        # HTML + JS UI (calls short.sctp-sandbox.com)
├── build.sh              # (Deprecated) Old ZIP Lambda build script
├── docker-build.sh       # Script to build and push Docker containers
├── docker-build.sh       # Script to build and push Docker containers
└── README.md
```
...
- ECR integration for container registry management
- Support for larger deployment packages and custom runtimes

```bash
# Build and deploy containerized functions
./docker-build.sh
```

### 🚀 **Ansible Automation**
- Automated deployment and configuration management
- Infrastructure health monitoring and reporting
- Multi-environment support with consistent processes
...
### 2. Build Lambda Docker Images

```bash
./docker-build.sh
```

This script builds and pushes Docker images for Lambda deployment to ECR.
